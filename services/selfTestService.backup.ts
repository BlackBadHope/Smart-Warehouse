...
class SelfTestService {
  ...
  async runFullTestSuite(): Promise<TestSuite[]> {
    ...
    try {
      ...
      // Emit test completion event to update UI
  ...
  private async runSmartieTests(): Promise<void> {
    ...
    await this.runTest(suite, 'Local LLM Configuration', async () => {
      const localLlmConfig = {
        baseUrl: 'http://192.168.222.135:5174', // SMARTIE local URL
        model: 'openai/gpt-oss-20b',
        apiKey: 'test-key' // Optional for local LLM
      };
      
      // Test connection to local LLM with timeout and better error handling
      try {
        ...
        if (!response.ok) {
          ...
          const altResponse = await fetch(`${localLlmConfig.baseUrl}/v1/models`, {
            method: 'GET',
          ...
          if (!altResponse.ok) {
            throw new Error(`Local LLM not reachable on any endpoint: ${response.status}`);
          }
        }
        
        const modelsData = response.ok ? await response.json() : { models: [] };
...
Execute all these actions and provide a confirmation of each step.`;
  ...
  private async runNetworkTests(): Promise<void> {
    ...
    // Test 2: Local Network Discovery (Enhanced with step-by-step logging)
    await this.runTest(suite, 'Local Network Discovery', async () => {
      const steps: any[] = [];
      try {
        // Step 1: Initialize discovery
        steps.push({ step: 1, action: 'Initializing discovery', timestamp: Date.now(), status: 'started', details: { networkServiceInitialized: networkService.getNetworkState().isOnline } });
        
        // Step 2: Check network interface
        const localDevice = networkService.getLocalDevice();
        steps.push({ step: 2, action: 'Checking network interface', timestamp: Date.now(), status: 'checking', details: { localDeviceId: localDevice.id, ipAddress: localDevice.ipAddress } });
        
        // Step 3: Validate network configuration  
        steps.push({ step: 3, action: 'Validating network configuration', timestamp: Date.now(), status: 'validating', details: { port: localDevice.port, capabilities: localDevice.capabilities } });
        
        // Step 4: Start discovery process
        steps.push({ step: 4, action: 'Starting discovery process', timestamp: Date.now(), status: 'starting', details: { discoveryMethod: 'mDNS simulation' } });
        
        // Step 5: Broadcasting discovery packets
        steps.push({ step: 5, action: 'Broadcasting discovery packets', timestamp: Date.now(), status: 'broadcasting', details: { packetType: 'discovery_announce' } });
        
        // Step 6: Monitor network traffic
        steps.push({ step: 6, action: 'Monitoring network responses', timestamp: Date.now(), status: 'monitoring', details: { timeout: '1000ms' } });
        
        // Step 7: Wait for responses
        steps.push({ step: 7, action: 'Waiting for device responses', timestamp: Date.now(), status: 'waiting' });
        await new Promise(resolve => setTimeout(resolve, 1000));
        
        // Step 8: Collect discovered devices
        steps.push({ step: 8, action: 'Collecting discovered devices', timestamp: Date.now(), status: 'collecting' });
        const discoveredDevices: any[] = []; // networkService.getDiscoveredDevices();
        
        // Step 9: Stop discovery
        steps.push({ step: 9, action: 'Stopping discovery process', timestamp: Date.now(), status: 'stopping' });
        ...
        return { 
          status: 'FAIL', 
          message: `Network discovery failed at step ${failedStep?.step}: ${(error as Error).message}`,
          details: { 
            discoverySteps: steps,
    ...
    await this.runTest(suite, 'Chat Commands System', async () => {
      try {
        ...
        for (const command of commands) {
          const steps: any[] = [];
          try {
            steps.push({ step: 1, action: `Parsing command: ${command}`, timestamp: Date.now(), status: 'started', details: { command, length: command.length } });
            
            steps.push({ step: 2, action: 'Validating command format', timestamp: Date.now(), status: 'validating', details: { isCommand: command.startsWith('/') } });
            
            steps.push({ step: 3, action: 'Loading warehouse data', timestamp: Date.now(), status: 'loading', details: { warehouseId: warehouse.id } });
            
            steps.push({ step: 4, action: 'Checking user permissions', timestamp: Date.now(), status: 'checking', details: { senderId: 'test-user' } });
            
            steps.push({ step: 5, action: 'Resolving command handler', timestamp: Date.now(), status: 'resolving', details: { commandName: command.split(' ')[0] } });
            
            steps.push({ step: 6, action: 'Executing command handler', timestamp: Date.now(), status: 'executing' });
            
            const result = await chatService.sendMessage(warehouse.id, command, 'text');
            
            steps.push({ step: 7, action: 'Processing command result', timestamp: Date.now(), status: 'processing', details: { resultType: result.type } });
            
            steps.push({ step: 8, action: 'Command completed successfully', timestamp: Date.now(), status: 'completed', details: { messageId: result.id } });
            ...
            const errorStep = { step: steps.length + 1, action: 'Command failed', timestamp: Date.now(), status: 'failed', error: (error as Error).message };
            ...
            commandResults.push({
              ...
              status: 'failed',
              executionSteps: steps,
              failedAtStep: errorStep.step
            });
            
            debugService.error(`Chat command test failed for ${command}:`, error);
          }
        }
        ...
        return { 
          ...
          details: { 
            commandResults,
            successfulCommands,
            failedCommands: commandResults.filter(r => r.status === 'failed').length,
            totalSteps: commandResults.reduce((sum, r) => sum + r.executionSteps.length, 0),
            completedSteps: commandResults.reduce((sum, r) => sum + r.executionSteps.filter((s: any) => s.status === 'completed').length, 0)
          }
        };
      } catch (error) {
        return { 
    ...
    await this.runTest(suite, 'Basic Encryption Operations', async () => {
      try {
        ...
        if (decrypted !== testData) {
          throw new Error(`Decrypted data mismatch: expected '${testData}', got '${decrypted}'`);
        }
        ...
        debugService.error('Encryption test error details:', error);
        return { 
          status: 'FAIL', 
  ...
  private async runAccessControlTests(): Promise<void> {
    ...
    await this.runTest(suite, 'User Role Management', async () => {
      try {
        ...
        users.forEach(user => {
          accessControlService.grantPermission(testWarehouse, user.id, user.role, 'owner-123');
          debugService.info('SelfTest: Granted permission', { userId: user.id, role: user.role });
        });
        ...
        // Test role updates
        accessControlService.grantPermission(testWarehouse, 'user-2', 'viewer', 'owner-123');
        const updatedRole = accessControlService.getUserRole(testWarehouse, 'user-2');
        debugService.info('SelfTest: Updated role', { userId: 'user-2', newRole: updatedRole });
